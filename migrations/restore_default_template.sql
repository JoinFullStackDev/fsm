-- Restore default template field configurations
-- This script will re-populate the "FullStack Method Default" template with all field configs
-- It's idempotent - won't create duplicates if fields already exist
...
begin
  ...
  if default_template_id is null then
    raise exception 'Default template "FullStack Method Default" not found. Please create it first.';
  end if;

  raise notice 'Restoring field configurations for template: %', default_template_id;
  ...
  select uuid_generate_v4(), default_template_id, 1, 'target_users', 'array', 2,
   '{"columns": 12}'::jsonb,
   '{"label": "Target Users", "helpText": "List the primary user segments or personas this product targets.", "aiSettings": {"enabled": true, "customPrompt": "Suggest target user segments based on the problem statement."}}'::jsonb
  where not exists (
    select 1 from template_field_configs
    where template_id = default_template_id
    and phase_number = 1
  ...
  select uuid_generate_v4(), default_template_id, 1, 'why_now', 'textarea', 3,
   '{"columns": 12}'::jsonb,
   '{"label": "Why Now?", "helpText": "Explain why this is the right time to build this product. Consider market timing, technology readiness, competitive landscape, etc."}'::jsonb
  where not exists (
    select 1 from template_field_configs
    where template_id = default_template_id
    and phase_number = 1
  ...
  select uuid_generate_v4(), default_template_id, 1, 'value_hypothesis', 'textarea', 4,
   '{"columns": 12}'::jsonb,
   '{"label": "Value Hypothesis", "helpText": "Describe the core value proposition and how you believe users will benefit."}'::jsonb
  where not exists (
    select 1 from template_field_configs
    where template_id = default_template_id
    and phase_number = 1
  ...
  select uuid_generate_v4(), default_template_id, 1, 'constraints', 'array', 5,
   '{"columns": 12}'::jsonb,
   '{"label": "Constraints", "helpText": "List any constraints (budget, time, technical, legal, etc.) that will impact development.", "aiSettings": {"enabled": true, "customPrompt": "Identify potential constraints for this product."}}'::jsonb
  where not exists (
    select 1 from template_field_configs
    where template_id = default_template_id
    and phase_number = 1
  ...
  select uuid_generate_v4(), default_template_id, 1, 'risks', 'array', 6,
   '{"columns": 12}'::jsonb,
   '{"label": "Risks", "helpText": "Identify potential risks and mitigation strategies.", "aiSettings": {"enabled": true, "customPrompt": "Identify potential risks for this product."}}'::jsonb
  where not exists (
    select 1 from template_field_configs
    where template_id = default_template_id
    and phase_number = 1
  ...
  select uuid_generate_v4(), default_template_id, 1, 'assumptions', 'array', 7,
   '{"columns": 12}'::jsonb,
   '{"label": "Assumptions", "helpText": "List key assumptions that need to be validated.", "aiSettings": {"enabled": true, "customPrompt": "Identify key assumptions for this product."}}'::jsonb
  where not exists (
    select 1 from template_field_configs
    where template_id = default_template_id
    and phase_number = 1
  ...
  select uuid_generate_v4(), default_template_id, 1, 'initial_features', 'array', 8,
   '{"columns": 12}'::jsonb,
   '{"label": "Initial Features", "helpText": "Very high-level feature list (bullet points)."}'::jsonb
  where not exists (
    select 1 from template_field_configs
    where template_id = default_template_id
    and phase_number = 1
  ...
  select uuid_generate_v4(), default_template_id, 1, 'feasibility_notes', 'textarea', 9,
   '{"columns": 12}'::jsonb,
   '{"label": "Feasibility Notes", "helpText": "Very rough technical feasibility notes."}'::jsonb
  where not exists (
    select 1 from template_field_configs
    where template_id = default_template_id
    and phase_number = 1
  ...
  raise notice 'Default template field configurations restored successfully!';
end $$;
