...
import { findAllRelatedContent } from '@/lib/kb/linking';
...
export async function GET(
  ...
  try {
    ...
    if (!authUser) {
      return unauthorized('You must be logged in to view related content');
    }
    ...
    // Get related content
    const relatedContent = await findAllRelatedContent(
      supabase,
      params.id,
      organizationId
    );
    ...
    logger.error('[KB Articles API] Exception in GET related:', error);
    return internalError('Failed to fetch related content');
  }
}
