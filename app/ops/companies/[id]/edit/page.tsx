...
export default function EditCompanyPage() {
    ...
    const loadPartners = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setPartnerOptions(data.filter((p: PartnerCompanyOption) => p.id !== companyId));
        }
      } catch (err) {
  ...
  const loadCompany = useCallback(async () => {
    try {
      ...
      setPartnerCommissionRate(company.partner_commission_rate?.toString() || '');
      setPartnerContactEmail(company.partner_contact_email || '');
      setPartnerNotes(company.partner_notes || '');
      setReferredByCompanyId(company.referred_by_company_id || '');
    } catch (err) {
